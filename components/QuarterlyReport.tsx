    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 print:hidden">
        {QUARTER_NAMES.map((qName, qIdx) => {
          ...
          return (
            <div key={qName} className="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-sm flex items-center gap-4">
              <div className="p-3.5 bg-slate-700 text-emerald-400 rounded-xl">
                <PieChart size={24} />
              </div>
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">{qName}</p>
                <p className="text-xl font-black text-white">Rp {formatIDR(qAmount)}</p>
      ...
      <div className="bg-slate-800 rounded-2xl shadow-xl border border-slate-700 overflow-x-auto overflow-y-auto max-h-[72vh] relative print:max-h-none print:overflow-visible print:shadow-none print:border-none">
        ...
        <table className="w-full text-left min-w-[1300px] print:min-w-0 print:text-[8px] border-collapse">
          <thead className="bg-slate-900 border-b border-slate-700 sticky top-0 z-20 shadow-sm">
            <tr>
              <th className="px-5 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest sticky top-0 left-0 bg-slate-900 z-30 shadow-sm">SKPD</th>
              {level === 'sub_kegiatan' && (
                <th className="px-4 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest sticky top-0 bg-slate-900 z-20">Kode SubKeg</th>
              )}
              <th className="px-5 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest min-w-[220px] sticky top-0 bg-slate-900 z-20">Uraian / Kode</th>
              <th className="px-4 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest text-right sticky top-0 bg-slate-900 z-20">Pagu Anggaran</th>
              <th className="px-4 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest text-right sticky top-0 bg-slate-900 z-20">Pagu SPD</th>
              
              {QUARTER_SHORT.map((qLabel) => (
                <th key={qLabel} className="px-3.5 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest text-right bg-slate-900 sticky top-0 z-20">
                  {qLabel}
                </th>
              ))}

              <th className="px-4 py-4 text-[10px] font-black text-emerald-400 uppercase tracking-widest text-right bg-slate-900 sticky top-0 z-20">Total Realisasi</th>
              <th className="px-4 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest text-right sticky top-0 bg-slate-900 z-20">Sisa SPD</th>
              <th className="px-4 py-4 text-[10px] font-black text-rose-400 uppercase tracking-widest text-right sticky top-0 bg-slate-900 z-20">Sisa Anggaran</th>
              <th className="px-4 py-4 text-[10px] font-black text-slate-300 uppercase tracking-widest text-center sticky top-0 bg-slate-900 z-20">% Capaian</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700 text-slate-200">
            {quarterlyData.map((row, idx) => {
              ...
              return (
                <tr key={idx} className={`hover:bg-slate-700/50 transition-colors group ${row.isUnmapped ? 'bg-[#4c0519]/30' : isOverSpd ? 'bg-amber-950/20' : ''}`}>
                  <td className="px-5 py-3.5 text-xs font-bold text-slate-300 sticky left-0 bg-slate-800 group-hover:bg-slate-700/70 z-10 shadow-sm">{row.skpd}</td>
                  {level === 'sub_kegiatan' && (
                    <td className="px-4 py-3.5 text-[10px] font-mono text-emerald-400 font-bold">{row.kode_sub_kegiatan || '-'}</td>
                  )}
                  <td className="px-5 py-3.5">
                    <div className="flex flex-col">
                      ...
                      <p className="text-xs font-bold text-white leading-tight max-w-[240px] truncate" title={row.name}>{row.name}</p>
                      {row.parentName && <p className="text-[8.5px] text-slate-400 uppercase font-medium truncate max-w-[240px]">{row.parentName}</p>}
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-xs font-bold text-right text-white">Rp {formatIDR(row.anggaran)}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-right text-emerald-400">Rp {formatIDR(row.pagu_spd)}</td>
                  ...
                  <td className="px-4 py-3.5 text-xs font-bold text-right text-emerald-400">
                    <button
                  ...
                  <td className={`px-4 py-3.5 text-xs font-bold text-right ${sisaSpd < 0 ? 'text-rose-400' : 'text-slate-300'}`}>Rp {formatIDR(sisaSpd)}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-right text-rose-400">Rp {formatIDR(sisaAnggaran)}</td>
            ...
            <tr className="bg-slate-900 text-white font-black sticky bottom-0 z-20 shadow-md">
              <td className="px-5 py-4 text-xs uppercase tracking-wider sticky bottom-0 left-0 bg-slate-900 z-30 shadow-md" colSpan={level === 'sub_kegiatan' ? 3 : 2}>
                Total Seluruhnya
              </td>
              <td className="px-4 py-4 text-xs text-right sticky bottom-0 bg-slate-900">Rp {formatIDR(totals.anggaran)}</td>
              <td className="px-4 py-4 text-xs text-right text-emerald-400 sticky bottom-0 bg-slate-900">Rp {formatIDR(totals.pagu_spd)}</td>
              {totals.quarterRealisasi.map((sumVal, qIdx) => (
                <td key={qIdx} className="px-3.5 py-4 text-xs text-right text-slate-300 font-mono sticky bottom-0 bg-slate-900">
                  {sumVal > 0 ? formatIDR(sumVal) : '-'}
                </td>
              ))}
              <td className="px-4 py-4 text-xs text-right text-emerald-400 sticky bottom-0 bg-slate-900">Rp {formatIDR(totals.totalRealisasi)}</td>
              <td className="px-4 py-4 text-xs text-right text-slate-300 sticky bottom-0 bg-slate-900">Rp {formatIDR(totals.pagu_spd - totals.totalRealisasi)}</td>
              <td className="px-4 py-4 text-xs text-right text-rose-400 sticky bottom-0 bg-slate-900">Rp {formatIDR(totals.anggaran - totals.totalRealisasi)}</td>
              <td className="px-4 py-4 text-center text-xs sticky bottom-0 bg-slate-900">
